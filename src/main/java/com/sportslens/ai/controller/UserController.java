...
import com.sportslens.ai.service.ArticleLikeService;
...
public class UserController {
    ...
    public UserController(UserService userService, UserRepository userRepository,
                          BrowsingHistoryRepository browsingHistoryRepository, StorageService storageService,ArticleLikeService articleLikeService) {
    ...
    @ResponseBody
    public ResponseEntity<Map<String, Object>> likeArticle(@PathVariable Long articleId, Principal principal) {
        if (principal == null) {
        ...
        Map<String, Object> result = articleLikeService.likeArticle(articleId, principal.getName());
    ...
    @ResponseBody
    public ResponseEntity<Map<String, Object>> unlikeArticle(@PathVariable Long articleId, Principal principal) {
        if (principal == null) {
            return ResponseEntity.status(401).body(Map.of(
                "success", false,
                "message", "请先登录"
        ...
        Map<String, Object> result = articleLikeService.unlikeArticle(articleId, principal.getName());
        return ResponseEntity.ok(result);
    }
     ...
     * 切换点赞状态（推荐使用这个接口）
     */
    @PostMapping("/article/{articleId}/toggle-like")
    @ResponseBody
    public ResponseEntity<Map<String, Object>> toggleLike(@PathVariable Long articleId, Principal principal) {
        if (principal == null) {
            return ResponseEntity.status(401).body(Map.of(
                "success", false,
                "message", "请先登录"
        ...
        Map<String, Object> result = articleLikeService.toggleLike(articleId, principal.getName());
        return ResponseEntity.ok(result);
    }
    ...
    @ResponseBody
    public ResponseEntity<Map<String, Object>> getArticleLikeStatus(@PathVariable Long articleId, Principal principal) {
        String username = principal != null ? principal.getName() : null;
        Map<String, Object> result = articleLikeService.getArticleLikeStatus(articleId, username);
        return ResponseEntity.ok(result);
    }
}